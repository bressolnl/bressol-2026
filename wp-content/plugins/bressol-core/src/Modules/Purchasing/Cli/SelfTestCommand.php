...
final class SelfTestCommand
{
    ...
    public function __invoke(array $args, array $assocArgs): void
    {
        ...
        \WP_CLI::log('Settings: purchasing_planning_cycle_days=' . (int) ($settings['purchasing_planning_cycle_days'] ?? 0));
        \WP_CLI::log('Settings: purchasing_planning_reminder_lead_days=' . (int) ($settings['purchasing_planning_reminder_lead_days'] ?? 0));
        $nextShipment = (string) ($settings['purchasing_planning_next_shipment_date_utc'] ?? '');
        \WP_CLI::log('Settings: purchasing_planning_next_shipment_date_utc=' . ($nextShipment !== '' ? $nextShipment : '(not set)'));
        \WP_CLI::log('Settings: purchasing_planning_run_on_close_window=' . (!empty($settings['purchasing_planning_run_on_close_window']) ? 'true' : 'false'));
        \WP_CLI::log('Cost ledger adapter available: ' . (CostLedgerAdapter::is_available() ? 'true' : 'false'));
        $store = new PlanningStore();
        \WP_CLI::log('Planning latest run: ' . ($store->get_latest() ? 'yes' : 'no'));
